                        ...
                        @if (isset($grid_section) && !empty($grid_section))
                            @foreach ($grid_section as $s)
                                <div class="managementsec_bannner">
                                    <img
                                        src="{{ asset(isset($s->gimage) ? 'images/' . $s->gimage : 'images/computerbanner.jpg') }}" />
                                </div>
                            @endforeach
                        @endif
                    </div>
    ...
    $(document).ready(function() {
                ...
                success: function(response) {
                    ...
                    industries.forEach(function(industry) {
                        ...
                        filters += '<div class="filter-option">' +
                            '<label class="tick">' +
                            industry.title +
                            '<input type="checkbox" class="industry-filter" data-industry-id="' +
                            industry.id + '" ' + isChecked + '>' +
                            '<span class="check"></span>' +
                            '</label>' +
                            '</div>'
                    });
        ...
        function displayProducts(products) {
            ...
            products.forEach(function(data) {
                ...
                const shortDesc = words.slice(0, wordLimit).join(' '); // First 'wordLimit' words
                const fullDesc = words.join(' '); // Full description

                // Check if we need to show "Show More" (only if there's more content than the limit)
                const showMoreNeeded = words.length > wordLimit;
        ...
        function loadIndustries1(products) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var filters = '';
                    industries.forEach(function(industry) {
                        var isChecked = selectedIndustries1.includes(industry.id) ?
                            'checked' : '';

                        filters += '<div class="filter-option">' +
                            '<label class="tick">' +
                            industry.title +
                            '<input type="checkbox" class="industry-filter1" data-industry-id1="' +
                            industry.id + '" ' + isChecked + '>' +
                            '<span class="check"></span>' +
                            '</label>' +
                            '</div>'
