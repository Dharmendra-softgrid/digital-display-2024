...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $pass_array = [];
            $columns = ["id", "title","model", "series", "featured_image"];
    ...
    public function store(Request $request)
    {
        ...
        $product->key_features = $request->keyfeatures;
        ...
        $serializedGetSavedVariant = array_map('serialize', $getSavedVariant);
       $productIds= $data['product_id'] ;
       $variantIds = (string)$data['variant'];
            //Delete records based on product_id and variant_id
            ProductComponent::where('product_id', $productIds)->where('variant_id', $variantIds)->delete();
            ProductSpecifications::where('product_id', $productIds)->where('variant_id', $variantIds)->delete();
            ProductBlog::where('product_id', $productIds)->where('variant_id', $variantIds)->delete();
            ProductKeyFeatureDescription::where('product_id', $productIds)->where('variant_id', $variantIds)->delete();
            ProductVariant::where('product_id',$productIds)->delete();
        }
            ...
            if ($request->has('v_id')) {
                ...
                foreach ($request->v_id as $variantId) {
                    ...
                    if ($request->hasFile("csv_file.$variantId")) {
                        ...
                        // code changes for deleting existing components starts here
                        DB::beginTransaction();

                        $existingRecords = ProductComponent::where('product_id', $product->id)
                        ->where('variant_id', $variantId)
                        ->exists();
                
                    if ($existingRecords) {
                        // Delete all existing records for the product and variant
                        ProductComponent::where('product_id', $product->id)
                            ->where('variant_id', $variantId)
                            ->delete();
                
                        // Log the deletion
                        Log::info("Existing ProductComponent records deleted for product_id: {$product->id}, variant_id: {$variantId}");
                    }
                        ...
                        foreach ($files as $file) {
                            ...
                            foreach ($records as $record) {
                                Log::info('Records action log:', $record);
